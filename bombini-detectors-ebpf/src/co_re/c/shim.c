...
#define _SHIM_CRED_CAP(cap_field)                                                         \
    __attribute__((always_inline)) __u64 shim_cred_##cap_field(struct cred *cred)         \
    {                                                                                     \
        if (bpf_core_field_exists(cred->cap_field.val)) {                                 \
            return BPF_CORE_READ(cred, cap_field.val);                                    \
        }                                                                                 \
        /* Old kernels: kernel_cap_t is struct kernel_cap_struct { __u32 cap[2] } */      \
        struct kernel_cap_struct *old =                                                   \
            (struct kernel_cap_struct *)__builtin_preserve_access_index(                  \
                &cred->cap_field);                                                        \
        __u32 lo = BPF_CORE_READ(old, cap[0]);                                            \
        __u32 hi = BPF_CORE_READ(old, cap[1]);                                            \
        return ((__u64)hi << 32) | lo;                                                    \
    }                                                                                     \
    _FIELD_EXISTS_DEF(cred, cap_field, cap_field)
...
#define SHIM_TRUSTED_OR_NULL(struc, memb)                                    \
    _SHIM_GETTER(                                                            \
        typeof(((struct struc *)0)->memb),                                   \
        shim_##struc##_##memb##_trusted_or_null(struct struc *struc),        \
        struc->memb)                                                         \
    _FIELD_EXISTS_DEF(struc, memb, memb##_trusted_or_null)
...
_SHIM_GETTER_BPF_CORE_READ(gid_t, shim_cred_fsuid(struct cred *cred), cred, fsuid.val);
_SHIM_GETTER_BPF_CORE_READ(gid_t, shim_cred_fsgid(struct cred *cred), cred, fsgid.val);

_SHIM_CRED_CAP(cap_effective)
_SHIM_CRED_CAP(cap_inheritable)
_SHIM_CRED_CAP(cap_permitted)
...
SHIM_TRUSTED_OR_NULL(dentry, d_inode);
