...
use bombini_common::{
    config::{
        procmon::ProcMonKernelConfig,
        rule::{CapKey, ExecArgKey, FileNameMapKey, PathMapKey, PathPrefixMapKey, Rules, UIDKey},
    },
    constants::{
        MAX_ARG_SIZE, MAX_ARGS_COUNT, MAX_ARGS_SIZE, MAX_FILE_PATH, MAX_FILE_PREFIX,
        MAX_FILENAME_SIZE, MAX_IMA_HASH_SIZE, PAGE_SIZE,
    },
    event::{
...
unsafe fn delete_arguments_callback(
    map: *mut c_void,
    key: *mut c_void,
    _: *mut c_void,
    _: *mut c_void,
...
#[map]
static PROCMON_EXECVE_SANDBOX_ARG_MAP: HashMap<ExecArgKey, [[u8; MAX_ARG_SIZE]; MAX_ARGS_COUNT]> =
    HashMap::with_max_entries(1, 0);

#[map]
static PROCMON_EXECVE_SANDBOX_BINPATH_MAP: HashMap<PathMapKey, u8> =
    HashMap::with_max_entries(1, 0);

#[map]
static PROCMON_EXECVE_SANDBOX_BINNAME_MAP: HashMap<FileNameMapKey, u8> =
    HashMap::with_max_entries(1, 0);
...
#[map]
static PROCMON_EXECVE_PBINPATH_MAP: HashMap<PathMapKey, u8> = HashMap::with_max_entries(1, 0);

#[map]
static PROCMON_EXECVE_PBINNAME_MAP: HashMap<FileNameMapKey, u8> = HashMap::with_max_entries(1, 0);

#[map]
static PROCMON_EXECVE_PBINPREFIX_MAP: LpmTrie<PathPrefixMapKey, u8> =
    LpmTrie::with_max_entries(1, 0);

// Execve sandbox helper maps
#[map]
static PROCMON_EXECVE_SANDBOX_PROCESS_ARGS_MAP: PerCpuHashMap<[u8; MAX_ARG_SIZE], u8> =
    PerCpuHashMap::with_max_entries(64, 0);

#[map]
static PROCMON_EXECVE_SANDBOX_ARGS_HELPER_MAP: PerCpuArray<[u8; MAX_ARG_SIZE]> =
    PerCpuArray::with_max_entries(1, 0);
...
fn try_execve_sandbox(
    ...
    unsafe {
        ...
        let Some(deny_list): Option<bool> =
            config.sandbox_mode[ProcessEventNumber::ExecveSandbox as usize]
        else {
            return Err(-1);
        };
        ...
        for (idx, rule) in rule_array.iter().take_while(|x| !x.is_empty()).enumerate() {
            ...
            let mut scope_filter = interpreter::Interpreter::new(ParentScopeFilter::new(
                ...
                &PROCMON_EXECVE_PBINNAME_MAP,
                &PROCMON_EXECVE_PBINPATH_MAP,
                &PROCMON_EXECVE_PBINPREFIX_MAP,
                binary_name,
                binary_path,
                binary_prefix,
                parent_name,
