...
use bombini_common::config::rule::{
    ...
    BpfProgTypeKey, CapKey, CreationFlagsKey, ExecArgKey, FileNameMapKey, FlagsKey, ImodeKey,
    Ipv4MapKey, Ipv6MapKey, PathMapKey, PathPrefixMapKey, PortKey, ProtModeKey, SocketFlagsKey,
    SocketTypeKey, UIDKey,
};
use bombini_common::constants::{
    MAX_ARG_SIZE, MAX_ARGS_COUNT, MAX_BPFNAME_SIZE, MAX_FILE_PATH, MAX_FILE_PREFIX,
    MAX_FILENAME_SIZE,
};
...
impl Attribute for ParentBinaryPathAttribute {
    ...
    fn store_attribute(
        ...
        let Some(map) = ebpf.map_mut(&format!("{map_name_prefix}_PBINPATH_MAP")) else {
            return Ok(());
        };
        let mut path_map: EbpfHashMap<_, PathMapKey, u8> = EbpfHashMap::try_from(map)?;
        for (path, value) in self.map.iter() {
            let mut key = PathMapKey {
                rule_idx,
                path: [0u8; MAX_FILE_PATH],
    ...
    fn get_attribute_map_size(&self, map_name_prefix: &str) -> (String, u32) {
        (
            format!("{map_name_prefix}_PBINPATH_MAP"),
            self.map.len() as u32,
        )
    }
}
...
impl Attribute for ParentBinaryNameAttribute {
    ...
    fn store_attribute(
        ...
        let Some(map) = ebpf.map_mut(&format!("{map_name_prefix}_PBINNAME_MAP")) else {
            return Ok(());
        };
        let mut name_map: EbpfHashMap<_, FileNameMapKey, u8> = EbpfHashMap::try_from(map)?;
        for (name, value) in self.map.iter() {
            let mut key = FileNameMapKey {
                rule_idx,
                name: [0u8; MAX_FILENAME_SIZE],
    ...
    fn get_attribute_map_size(&self, map_name_prefix: &str) -> (String, u32) {
        (
            format!("{map_name_prefix}_PBINNAME_MAP"),
            self.map.len() as u32,
        )
    }
}
...
impl Attribute for ParentBinaryPrefixAttribute {
    ...
    fn store_attribute(
        ...
        let Some(map) = ebpf.map_mut(&format!("{map_name_prefix}_PBINPREFIX_MAP")) else {
            return Ok(());
        };
        let mut prefix_map: LpmTrie<_, PathPrefixMapKey, u8> = LpmTrie::try_from(map)?;
        for (prefix, value) in self.map.iter() {
            let mut key = PathPrefixMapKey {
                rule_idx,
                path_prefix: [0u8; MAX_FILE_PREFIX],
    ...
    fn get_attribute_map_size(&self, map_name_prefix: &str) -> (String, u32) {
        (
            format!("{map_name_prefix}_PBINPREFIX_MAP"),
            self.map.len() as u32,
        )
    }
}
...
impl Attribute for ArgAttribute {
    fn serialize(&mut self, values: &[Literal], in_idx: u8) -> Result<(), anyhow::Error> {
        ...
        if values.len() > MAX_ARGS_COUNT {
            bail!(
                "Too many arguments for IN operation, max is {}",
                MAX_ARGS_COUNT
            );
        }
        if values.iter().any(|x| x.len() > MAX_ARG_SIZE) {
    ...
    fn store_attribute(
        ...
        let mut arg_map: EbpfHashMap<_, ExecArgKey, [[u8; MAX_ARG_SIZE]; MAX_ARGS_COUNT]> =
            EbpfHashMap::try_from(
        ...
        for (in_idx, value) in self.map.iter() {
            ...
            args.iter_mut().zip(value).for_each(|(dst, src)| {
