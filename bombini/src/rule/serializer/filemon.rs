...
use bombini_common::config::rule::{
    AccessModeKey, CreationFlagsKey, FileOpenAttributes, FlagsKey, ImodeKey, MmapFileAttributes,
    PathChmodAttributes, ProtModeKey,
};
...
#[derive(Debug)]
pub struct MmapFilePredicate {
    pub predicate: Predicate,
    pub path_map: HashMap<String, u8>,
    pub path_prefix_map: HashMap<String, u8>,
    pub name_map: HashMap<String, u8>,
    pub prot_map: HashMap<u8, ProtMode>,
    pub flags_map: HashMap<u8, SharingType>,
}

impl MmapFilePredicate {
    pub fn new() -> Self {
        Self {
            predicate: [RuleOp::Fin; MAX_RULE_OPERATIONS],
            name_map: HashMap::new(),
            path_map: HashMap::new(),
            path_prefix_map: HashMap::new(),
            prot_map: HashMap::new(),
            flags_map: HashMap::new(),
        }
    }
}

impl Default for MmapFilePredicate {
    fn default() -> Self {
        Self::new()
    }
}
...
impl PredicateSerializer for MmapFilePredicate {
            ...
            return match name {
                "prot_mode" => {
                    let values: Result<Vec<ProtMode>, anyhow::Error> = values
                        ...
                        .cloned()
                        .map(|lit| match lit {
                            Literal::String(s) => ProtMode::from_str(&s).map_err(|x| {
                                anyhow::anyhow!("Error while parsing prot mode: {}", x)
                            }),
                            Literal::Uint(i) => Err(anyhow::anyhow!(
                                "expected String literal, found Uint: {}",
                                i
                ...
                "flags" => {
                    let values: Result<Vec<SharingType>, anyhow::Error> = values
                        ...
                        .cloned()
                        .map(|lit| match lit {
                            Literal::String(s) => SharingType::from_str(&s).map_err(|x| {
                                anyhow::anyhow!("Error while parsing sharing type flags: {}", x)
                            }),
                            Literal::Uint(i) => Err(anyhow::anyhow!(
                                "expected String literal, found Uint: {}",
                                i
                    ...
                    if self.flags_map.insert(in_idx, flags).is_some() {
                        bail!("sharing type flags already set for index {}", in_idx);
                    }
    ...
    fn store_attributes(
        ...
        for (in_idx, value) in self.prot_map.iter() {
            let key = ProtModeKey {
                rule_idx,
                in_idx: *in_idx,
            };
        ...
        for (in_idx, value) in self.flags_map.iter() {
            let key = FlagsKey {
                rule_idx,
                in_idx: *in_idx,
            };
    ...
    fn attribute_map_sizes(&self, map_name_prefix: &str) -> HashMap<String, u32> {
        ...
        map.insert(
            format!("{}_FLAGS_MAP", map_name_prefix),
            self.flags_map.len() as u32,
        );
        map
    }
}
