...
use crate::proto::config::{NetMonConfigNew, Rule};
    ...
    fn load_and_attach_programs(&mut self) -> Result<(), EbpfError> {
        ...
        if self.egress.is_some() {
            let tcp_v4_connect: &mut FExit = self
                .ebpf
                .program_mut("tcp_v4_connect_new")
                .unwrap()
                .try_into()?;
            ...
            let tcp_v6_connect: &mut FExit = self
                .ebpf
                .program_mut("tcp_v6_connect_new")
                .unwrap()
                .try_into()?;
        ...
        if self.ingress.is_some() {
            let tcp_accept: &mut FExit = self
                .ebpf
                .program_mut("inet_csk_accept_new")
                .unwrap()
                .try_into()?;
